...
$(function(){
        ...
        if(data!=null && data.length>0){
            ...
            for(var j=0;j<data.length;j++){
                ...
                html +=	'		        </div>'
                    ...
                    +'	            <div><span>所属行业：'+ (occupation_name)  +'</span></div>'
                    +'			    <div>发布时间：'+ data[j].released_time  +' </div>'
            ...
            success: function(ret){
                ...
                if(ret){
                    ...
                    if(ret.recordsTotal>0){
                        ...
                        $('#catalog-list').html('<li><div class="no-data">没有符合条件的数据目录</div></li>');
                        $('.pagination').hide();
                    }
                }else{
                    $("#total_count").text(0);
    ...
    //开始加载第一页数据，并初始化总数据
    // loadData(0,true);
    loadLeft(false,{'group_id':cur_subjectId});
    _leftFilter = {'group_id':cur_subjectId};
    loadData(0,false);
